...
public class TreeUtil {
	...
	public List<TreeObject> getChildTreeObjects(List<TreeObject> list, String parentId) {
		...
		for (Iterator<TreeObject> iterator = list.iterator(); iterator.hasNext();) {
			...
			if (StringUtils.isNotBlank(t.getParentId()) && StringUtils.isNotBlank(parentId)
					&& t.getParentId().equals(parentId)) {
				recursionFn(list, t);
				returnList.add(t);
			}
		}
	...
	private List<TreeObject> getChildList(List<TreeObject> list, TreeObject t) {
		...
		while (it.hasNext()) {
			TreeObject n = (TreeObject) it.next();
			if (StringUtils.isNotBlank(n.getParentId()) && StringUtils.isNotBlank(t.getId())
					&& n.getParentId().equals(t.getId())) {
				tlist.add(n);
			}
		}
		return tlist;
	...
	public List<TreeObject> getChildTreeObjects(List<TreeObject> list, String typeId, String prefix) {
		...
		for (Iterator<TreeObject> iterator = list.iterator(); iterator.hasNext();) {
			...
			// 一、根据传入的某个父节点ID,遍历该父节点的所有子节点
			if (StringUtils.isNotBlank(node.getParentId()) && StringUtils.isNotBlank(typeId)
					&& node.getParentId().equals(typeId)) {
