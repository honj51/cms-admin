...
$(function() {
	...
	grid = $("#jqGrid").jqGrid({
		url: rootPath + '/website/advertisement/findByPage.shtml',
		mtype: "POST",
		postData: {
		},
		styleUI: 'Bootstrap',
...
function add(){
	parent.layer.open({
		...
		type: 2,
		area: ["60%", "70%"],
		content: rootPath + '/website/advertisement/addUI.shtml',
		end : function(){
...
function edit(){
	...
	parent.layer.open({
		...
		type: 2,
		area: ["60%", "70%"],
		content: rootPath + '/website/advertisement/editUI.shtml?id=' + rowId,
		end : function(){
			$("#jqGrid").trigger('reloadGrid');
	    }
	});
...
function del() {
	...
	parent.layer.confirm('确定要删除广告吗？', function(index) {
		...
		$.ajax({
			type : "post",
			url : rootPath + '/website/advertisement/delete.shtml',
			data : {
				ids : rowIds.toString()
			},
			dataType : "json",
