...
$(function() {
	...
	grid = $("#jqGrid").jqGrid({
		url: rootPath + '/website/group/findByPage.shtml',
		mtype: "POST",
		postData: {
		},
		styleUI: 'Bootstrap',
...
function add(){
	parent.layer.open({
		...
		type: 2,
		area: ["60%", "80%"],
		content: rootPath + '/website/group/addUI.shtml',
		end : function(){
...
function edit(){
	...
	parent.layer.open({
		...
		type: 2,
		area: ["60%", "80%"],
		content: rootPath + '/website/group/editUI.shtml?id=' + rowId,
		end : function(){
			$("#jqGrid").trigger('reloadGrid');
	    }
	});
...
function del() {
	...
	parent.layer.confirm('确定要删除介绍吗？', function(index) {
		top.ajaxLoading();
		$.ajax({
			type : "post",
			url : rootPath + '/website/group/delete.shtml',
			data : {
				ids : rowIds.toString()
			},
			dataType : "json",
