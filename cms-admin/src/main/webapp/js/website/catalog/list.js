...
$(function() {
	...
	grid = $("#jqGrid").jqGrid({
		url: rootPath + '/website/catalog/findByPage.shtml',
		mtype: "POST",
		postData: {
		},
		styleUI: 'Bootstrap',
...
function add(){
	parent.layer.open({
		...
		type: 2,
		area: ["60%", "70%"],
		content: rootPath + '/website/catalog/addUI.shtml',
		end : function(){
...
function edit(){
	...
	parent.layer.open({
		...
		type: 2,
		area: ["60%", "70%"],
		content: rootPath + '/website/catalog/editUI.shtml?id=' + rowId,
		end : function(){
			$("#jqGrid").trigger('reloadGrid');
	    }
	});
...
function del() {
	...
	parent.layer.confirm('确定要删除目录吗？', function(index) {
		top.ajaxLoading();
		$.ajax({
			type : "post",
			url : rootPath + '/website/catalog/delete.shtml',
			data : {
				ids : rowIds.toString()
			},
			dataType : "json",
