...
$(function() {
	...
	grid = $("#jqGrid").jqGrid({
		url: rootPath + '/building/type/findByPage.shtml',
		mtype: "POST",
		postData: {
		},
		styleUI: 'Bootstrap',
...
function add(){
	parent.layer.open({
		...
		type: 2,
		area: ["60%", "60%"],
		content: rootPath + '/building/type/addUI.shtml',
		end : function(){
...
function edit(){
	...
	parent.layer.open({
		...
		type: 2,
		area: ["60%", "60%"],
		content: rootPath + '/building/type/editUI.shtml?id=' + rowId,
		end : function(){
			$("#jqGrid").trigger('reloadGrid');
	    }
	});
...
function del() {
	...
	parent.layer.confirm('确定要删除类型名称吗？', function(index) {
		top.ajaxLoading();
		$.ajax({
			type : "post",
			url : rootPath + '/building/type/delete.shtml',
			data : {
				ids : rowIds.toString()
			},
			dataType : "json",
