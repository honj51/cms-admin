...
$(function() {
	// -----------------------初始化grid---------------------------/
	 $("#jqGrid").jqGrid({
		url : rootPath + '/basedata/meas/findByPage.shtml',
		mtype : "GET",
		multiselect: true,
...
function addMeas() {
	pageii = parent.layer.open({
		...
		type : 2,
		area : [ "60%", "40%" ],
		content : rootPath + '/basedata/meas/addUI.shtml',
		end:function(){ 
...
function editMeas(){
	...
	pageii = parent.layer.open({
		...
		type : 2,
		area : [ "60%", "40%" ],
		content : rootPath + '/basedata/meas/editUI.shtml?number='+celValue,
		end:function(){ 
			$("#jqGrid").trigger("reloadGrid")
		}
	});
...
if(selRowId==null||selRowId==""){
	...
	parent.layer.confirm('是否删除？', function(index) {
		var url = rootPath + '/basedata/meas/deleteEntity.shtml?ids='+rowIds;
