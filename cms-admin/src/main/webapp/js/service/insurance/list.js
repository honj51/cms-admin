...
$(function() {
	...
	grid = $("#jqGrid").jqGrid({
		url: rootPath + '/service/insurance/findByPage.shtml',
		mtype: "get",
		postData: {
			status : status,
		},
...
$("#reject").click("click",function(){
	...
	parent.layer.open({
		...
		type : 2,
		area : [ "58%", "50%" ],
		content : rootPath + '/service/insurance/rejectUI.shtml?id=' + rowIds[0],
		maxmin: true,
		end : function(){
	...
	$("#pass").click("click",function(){
		...
		if(status == '已批准'){
		 ...
		 $.ajax({
			    type: "post",
			    url: rootPath + '/service/insurance/pass.shtml',
			    data: {
			        id: rowIds[0]
			    },
			    dataType: "json",
	...
	$("#preview").click("click",function(){
		...
		parent.layer.open({
			...
			type : 2,
			area : [ "70%", "80%" ],
			content : rootPath + '/service/insurance/previewUI.shtml?id=' + rowIds[0],
			maxmin: true,
			end : function(){
				$("#jqGrid").trigger('reloadGrid');
		    }
...
function exportExcel() {
	...
	var status = $("#status option:selected").val();
	window.location.href = rootPath + '/service/insurance/exportExcel.shtml?quaryParam='+quaryParam+'&status='+status;
}
function deleteInsurance() {
	...
	parent.layer.confirm('确定要删除信息吗？', function(index) {
		top.ajaxLoading();
		 $.ajax({
			    type: "post",
			    url: rootPath + '/service/insurance/deleteEntity.shtml',
			    data: {
			        ids: rowIds.toString()
			    },
			    dataType: "json",
