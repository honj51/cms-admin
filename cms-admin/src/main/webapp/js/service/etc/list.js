...
$(function() {
	...
	grid = $("#jqGrid").jqGrid({
		url: rootPath + '/service/etc/findByPage.shtml',
		mtype: "get",
		postData: {
			status : status,
		},
...
$("#reject").click("click",function(){
	...
	parent.layer.open({
		...
		type : 2,
		area : [ "58%", "50%" ],
		content : rootPath + '/service/etc/rejectUI.shtml?id=' + rowIds[0],
		maxmin: true,
		end : function(){
	...
	$("#pass").click("click",function(){
		...
		if (rowIds.length > 1) {
		 ...
		 $.ajax({
			    type: "post",
			    url: rootPath + '/service/etc/pass.shtml',
			    data: {
			        id: rowIds[0]
			    },
			    dataType: "json",
	...
	$("#preview").click("click",function(){
		...
		parent.layer.open({
			...
			type : 2,
			area : [ "70%", "80%" ],
			content : rootPath + '/service/etc/previewUI.shtml?id=' + rowIds[0],
			maxmin: true,
			end : function(){
				$("#jqGrid").trigger('reloadGrid');
		    }
...
function exportExcel() {
	...
	var quaryParam = $("#quaryParam").val().trim();
	var status = $("#status option:selected").val();
	window.location.href = rootPath + '/service/etc/exportExcel.shtml?quaryParam='+quaryParam+'&status='+status;
}
function deleteEtc() {
	var rowIds = $("#jqGrid").jqGrid('getGridParam', "selarrrow");
	if (rowIds.length == 0) {
	...
	parent.layer.confirm('确定要删除信息吗？', function(index) {
		top.ajaxLoading();
		 $.ajax({
			    type: "post",
			    url: rootPath + '/service/etc/deleteEntity.shtml',
			    data: {
			        ids: rowIds.toString()
			    },
			    dataType: "json",
