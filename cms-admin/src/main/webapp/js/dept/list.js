...
$(function() {
	...
	grid = $("#jqGrid").jqGrid({
		url: rootPath + '/dept/findByPage.shtml',
		mtype: "POST",
		postData: {
		},
		styleUI: 'Bootstrap',
...
function edit(){
	...
	parent.layer.open({
		...
		type: 2,
		area: ["60%", "70%"],
		content: rootPath + '/dept/editUI.shtml?id=' + rowId,
		end : function(){
			$("#jqGrid").trigger('reloadGrid');
	    }
	});
...
function del() {
	...
	parent.layer.confirm('确定要删除部门吗？', function(index) {
		top.ajaxLoading();
		$.ajax({
			type : "post",
			url : rootPath + '/dept/delete.shtml',
			data : {
				ids : rowIds.toString()
			},
			dataType : "json",
			...
			success : function(response) {
				...
				} else if(response == "have_roles"){
					parent.layer.msg('删除失败，部门下存在角色！', {
						icon : 1,
						time : 1000
					});
				} else if(response == "have_users"){
					parent.layer.msg('删除失败，部门下存在用户！', {
						icon : 1,
						time : 1000
					});
				} else {
...
function permissions() {
	...
	var url = rootPath + '/dept/permissions.shtml?id=' + ids[0];
...
function locked() {
	...
	parent.layer.confirm('确定要停用部门吗？', function(index) {
		top.ajaxLoading();
		$.ajax({
			type : "post",
			url : rootPath + '/dept/locked.shtml',
			data : {
				ids : rowIds.toString()
			},
			dataType : "json",
...
function unlocked() {
	...
	parent.layer.confirm('确定要启用部门吗？', function(index) {
		top.ajaxLoading();
		$.ajax({
			type : "post",
			url : rootPath + '/dept/unlocked.shtml',
			data : {
				ids : rowIds.toString()
			},
			dataType : "json",
