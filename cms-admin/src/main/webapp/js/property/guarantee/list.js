...
$(function() {
	...
	grid = $("#jqGrid").jqGrid({
		url: rootPath + '/property/guarantee/findByPage.shtml',
		mtype: "POST",
		postData: {
		},
		styleUI: 'Bootstrap',
...
function search(){
	grid.jqGrid('setGridParam', {
		postData: {
			...
			user_name : $("#queryparam").val().trim(),
			user_phone : $("#userPhone").val().trim(),
			type_name : $("#typeState").val().trim(),
		},
		page: 1
	}).trigger("reloadGrid");
}
...
function audit(){
	...
	parent.layer.open({
		...
		type: 2,
		area: ["70%", "70%"],
		content: rootPath + '/property/guarantee/auditUI.shtml?id=' + rowId,
		end : function(){
			$("#jqGrid").trigger('reloadGrid');
	    }
	});
