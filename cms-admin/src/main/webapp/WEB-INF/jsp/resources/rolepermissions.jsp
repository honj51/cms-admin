...
<script type="text/javascript">
	...
	function sub() {
		ly.ajax({
			...
			url : rootPath + '/shipper/resources/addRoleRes.shtml',
			dataType : 'json',
...
<body>
	<div class="choice_condition_menu" id="fixtop">
		<a href="javascript:void(0);" class="btn btn-sm btn-primary"
			onclick="sub()">保存</a>
		<!-- <a href="javascript:void(0);" class="btn btn-sm btn-primary" onclick="closeWin();">关闭</a> -->
	</div>
	<form method="post" id="from" name="form">
		...
		<table id="mytable" cellspacing="0"  style="border-bottom: 1px;border-right: 1px;"
			...
			<c:forEach items="${permissions}" var="k">
				<tr>
					<th scope="row" abbr="L2 Cache" class="specalt leftWidth"><input
					...
					<th scope="row" abbr="L2 Cache" class="specalt">
						<table id="mytable" cellspacing="0"
							...
							<c:forEach items="${k.children}" var="kc">
								<tr>
									<th scope="row" abbr="L2 Cache" class="specalt centerWidth"><input
									...
									<th><c:if test="${not empty kc.children}">
											...
											<table id="mytable" cellspacing="0"
												...
												<c:forEach items="${kc.children}" var="kcc">
													<tr>
														<th scope="row" abbr="L2 Cache" class="specalt rightWidth"><input
	...
	<script type="text/javascript">
		...
		$.ajax({
			type : "POST",
			data : {
				"shipperResRoleFormMap.userId" : "${param.userId}",
				"shipperResRoleFormMap.roleId" : "${param.roleId}"
			},
			url : rootPath + '/shipper/resources/findRoleRes.shtml',
			dataType : 'json',
			success : function(json) {
				for (index in json) {
					$(
